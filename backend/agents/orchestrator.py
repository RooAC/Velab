...
# 关于助手自身的元问题：与车辆诊断完全无关，直接由 orchestrator 短路回复，
# 不消耗 router-model token，也避免 LLM 误判为日志分析任务。
META_QUERY_REPLY = """您好！我是 **Velab FOTA 智能诊断助手**，可以帮您分析车辆固件升级（OTA / FOTA）相关问题。

我能做的事：
- 🔍 分析上传的 FOTA 升级日志（iCGM / MPU / IVI / MCU / IPK / T-BOX 等 ECU）
- 🧩 关联历史 Jira 工单与技术文档，定位根因
- 🛠 给出可操作的修复建议与下一步排查方向

请描述具体故障现象、涉及 ECU、错误码或大致发生时间，我会为您启动诊断。"""

# 极简元问题正则：长度 ≤ 30 字 且 命中 "你是/你叫/你能/你的/介绍.{0,3}你/什么模型/哪个模型"
# 之所以加长度阈值，是为了避免误命中正常诊断问题中包含 "你能查一下..." 这类短语
_META_QUERY_PATTERNS = (
    re.compile(r"你是(谁|什么|哪|啥)"),
    re.compile(r"你叫(什么|啥)"),
    re.compile(r"你能(做|干|帮|提供|分析)什么"),
    re.compile(r"你(是|用的是)?\s*(什么|哪个|哪种)\s*模型"),
    re.compile(r"你的(版本|名字|功能|能力|价格|收费)"),
    re.compile(r"介绍.{0,6}(你|自己)"),
    re.compile(r"你好[，,。\s]*$"),  # 纯打招呼
    re.compile(r"^(在吗|在么|你好|您好|hi|hello)[!！?？.。]*$", re.IGNORECASE),
)


def _is_meta_query(user_message: str) -> bool:
    """判断是否为关于助手自身的元问题或纯寒暄，命中则跳过 LLM 直接回复。"""
    if not user_message:
        return False
    text = user_message.strip()
    if len(text) > 30:
        return False
    return any(p.search(text) for p in _META_QUERY_PATTERNS)


# ── 模糊诊断提问检测 ──
# 用户表达想要诊断但没给具体线索（无 ECU、错误码、时间锚点、故障描述）。
# 命中后由 orchestrator 直接返回引导式澄清，避免 Agent 在零信息下空跑。
_VAGUE_VERB_RE = re.compile(
    r"(检查|看看|看下|查看|分析|诊断|看一下|帮我看|帮我查|看看问题|查下|看看这)"
)
_VAGUE_OBJECT_RE = re.compile(r"(问题|情况|故障|毛病|状况|这台车|这个车|下这|一下车|下日志)")
# 具体错误/现象词，命中一个则不视为模糊
_SPECIFIC_HINT_RE = re.compile(
    r"(升级|刷写|刷机|FOTA|OTA|失败|卡住|卡顿|超时|错误|异常|死机|黑屏|重启|重试|回滚|交互失败|下载失败|校验失败|签名|源文件|报错|报警)",
    re.IGNORECASE,
)
# ECU 名称、错误码、时间锐点
...
def _is_vague_diagnosis_query(user_message: str) -> bool:
    """判断是否为「想诊断但没给线索」的模糊提问。

    启动条件（需同时满足）：
      1. 长度 ≤ 30 字（镜像 meta-query 阈值，避免误伤长描述）
      2. 含泛化动词（检查/看看/分析/诊断）或泛化宾语（问题/情况/这台车）
      3. 未命中任何具体锐点：ECU 名/错误码/时间/具体现象词
    """
    if not user_message:
        return False
    text = user_message.strip()
    if len(text) > 30 or len(text) < 4:
        return False
    has_vague = bool(_VAGUE_VERB_RE.search(text) or _VAGUE_OBJECT_RE.search(text))
    if not has_vague:
        return False
    has_specific = bool(_SPECIFIC_HINT_RE.search(text) or _SPECIFIC_ANCHOR_RE.search(text))
    return not has_specific


VAGUE_QUERY_REPLY = """收到您的诊断请求，为了更准确定位问题，请补充以下信息：

1. **故障现象**：具体是什么表现？（如：升级失败 / 卡在 30% / 重启后黑屏 / 某个功能异常）
2. **涉及 ECU**：iCGM / MPU / IVI / MCU / IPK / T-BOX / 网关 中的哪一个？
3. **发生时间**：今天几点 / 昨天上午 / 具体日期时间点？
4. **错误信息**：是否有错误码、仪表提示或诊断报告？
5. **是否完成**：本次 OTA 是完全失败还是部分成功？

描述越具体，我能定位的根因越准确。如果已上传日志包，请一并告知大致时间范围、升级包名称或 ECU。"""
...
规则（重要）：
- **何时不要调用 Agent**（必须直接回复，**禁止** function call）：
  1. 寒暄 / 问候 / 闲聊（"你好"、"谢谢"、"在吗"、"嗯"）
  2. **关于助手自身的元问题**（"你是谁"、"你是什么模型"、"你叫什么名字"、"你能做什么"、"你的版本"、"你怎么收费"、"介绍一下你"等）—— 这类问题与车辆诊断无关，应简短自我介绍并引导。
  3. 用户已上传日志包但**消息本身未提供任何定位信息**（既无现象描述、又无 ECU、又无时间窗、又无错误码，例如只说"帮我看看"、"检查下这台车的问题"、"分析一下"）—— **即使已有 bundle 上下文，也应先反问澄清**，不要盲扫全量日志。
  4. 用户尚未提供可诊断的实质信息（无具体现象、无报错、无涉及模块/ECU、无日志或场景描述）。
- **何时必须调用 Agent**：用户已描述可分析的 FOTA/刷写/日志/工单类**具体**问题（至少包含 ECU/现象/错误码/时间窗 之一）时，应使用 function call；可同时调用多个 Agent（并行）。
...
async def orchestrate(
    ...
    # ── Meta-query 短路：关于助手自身的问题或纯寒暄，直接返回固定回复，不调 LLM ──
    if _is_meta_query(user_message):
        chain_debug(log, step="orchestrate.branch", event="META_QUERY_SHORTCUT")
        yield {
            "type": "step_start",
            "step": {
                "stepNumber": 1,
        ...
        yield {
            ...
            "step": {
                ...
                "status": "completed",
                "statusText": "Parallely Orchestrating...",
                "result": "识别为关于助手自身的元问题，直接回复，未调用诊断 Agent。",
            },
        }
        async for ev in _yield_clarification_stream(META_QUERY_REPLY):
            yield ev
        chain_debug(
            log,
            step="orchestrate",
    ...
    # ── 模糊诊断提问短路：「检查下这台车」「看看有什么问题」这类零信息请求──
    # 不调 Agent，直接返回引导式澄清提问，避免 Log Analytics 在零输入下空跑 + RCA 啦啦的结论。
    if _is_vague_diagnosis_query(user_message):
        chain_debug(log, step="orchestrate.branch", event="VAGUE_QUERY_SHORTCUT")
        yield {
            "type": "step_start",
            "step": {
                "stepNumber": 1,
        ...
        yield {
            ...
            "step": {
                ...
                "status": "completed",
                "statusText": "Parallely Orchestrating...",
                "result": "识别为信息不足的模糊提问，未调用诊断 Agent，引导用户补充现象、ECU、时间。",
            },
        }
        async for ev in _yield_clarification_stream(VAGUE_QUERY_REPLY):
            yield ev
        chain_debug(
            log,
            step="orchestrate",
